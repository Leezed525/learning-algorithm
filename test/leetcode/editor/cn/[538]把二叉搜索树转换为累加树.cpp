...
public:
    ...
    void recursion_GST(TreeNode *root, int &num) {
        if (root == nullptr)return;
        recursion_GST(root->right, num);
        num += root->val;
        root->val = num;
        recursion_GST(root->left, num);
    }

    TreeNode *convertBST(TreeNode *root) {
        int num = 0;
        return root == nullptr ? nullptr : (recursion_GST(root, num), root);
    }
};
