...
//
// Related Topics 字典树 数组 哈希表 字符串 👍 52 👎 0
...
public:
    ...
    string find_prefix(string word) {
        ...
        int i = 0;
        string res = "";
        while (i < word.size() && !tmp->isEnd) {
            if (tmp->next[word[i] - 'a']) tmp = tmp->next[word[i] - 'a'];
            else return word;
            res += word[i];
            i += 1;
        }
    ...
    string replaceWords(vector<string> &dictionary, string sentence) {
        ...
        while (r < sentence.size()) {
            if (sentence[r] == ' ') {
                string tmp = sentence.substr(l, r - l);
//                cout << tmp << endl;
                res += find_prefix(tmp) + " ";
                l = r + 1;
            }
        ...
        res += find_prefix(sentence.substr(l, r - l));
